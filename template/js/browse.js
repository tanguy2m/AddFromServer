$(function(){
  ...
  $("a.item").click(function() {
    if ($(this).closest('tr').find("img:first").attr('alt') == "dir")
      parent.hideBrowser(); // Quand un dossier est ouvert, on ferme la fenêtre parente, car le calcul des md5 peut être long
  });
  ...
  // -----------------------------------------
	md5_list = '';
	$("td.md5.row").each(function (index) {	md5_list += $(this).attr('id')+';';	}); // Fait par le client car temps masqué
	if(md5_list != '') { // Si il y a au moins une photo, requête
		$.ajax({ 
		  url: './../../../ws.php?format=json', // Remontée jusqu'à la racine de Piwigo
		  data: { method: 'pwg.images.exist',
        ...
        if (jQuery.parseJSON(data).stat == "ok") { // Si la requête n'a pas échoué
          $("td.md5.row").each(function (index) { // Boucle sur toutes les cellules de type 'md5'
            result = jQuery.parseJSON(data).result[$(this).attr('id')];
            if ( result > 0 ) {
